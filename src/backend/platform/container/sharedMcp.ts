...
import { toContainerPath, toEngineHostPath } from '../core/platform.js';
...
export async function ensureSharedMcpRunning(repoRoot: string): Promise<void> {
  ...
  const config = await getPlatformConfig(repoRoot);
  const healthUrl = `http://127.0.0.1:${config.mcp_port}/health`;
...
async function runSharedMcpBootstrap(
  ...
  const env = await createSharedMcpComposeBootstrapEnv(config.mcp_port, repoRoot);
  if (runtime.requiresComposeFile) {
    ...
    const overrideFile = await generateSharedMcpComposeOverride(
      ...
      { engineHost: runtime.engineHost, wslDistro: runtime.wslDistro },
    );
...
export async function generateSharedMcpComposeOverride(
  ...
  engineOptions: EngineHostPathOptions = {},
): Promise<string> {
  const overridePath = path.join(repoRoot, SHARED_MCP_COMPOSE_OVERRIDE_PATH);
  await validateExternalMountRoots(externalRoots);
  await writeTextFileAtomic(overridePath, renderSharedMcpComposeOverride(externalRoots, engineOptions));
  return overridePath;
}
...
// Bootstrap env for a compose build, overlaying the enterprise Python base-image
// override from repo .env so a value present only in .env still reaches compose
// build.args. process.env wins; TASKSAIL_PYTHON_BASE_IMAGE is intentionally not
// in SCRUBBED_SHARED_MCP_ENV_KEYS, so a process.env value is already carried by
// createSharedMcpBootstrapEnv — only the .env-only case is filled in here. Reads
// .env via core loadEnv to avoid a container -> setup layer dependency.
export async function createSharedMcpComposeBootstrapEnv(
  mcpPort: number,
  repoRoot: string,
  baseEnv: NodeJS.ProcessEnv = process.env,
): Promise<NodeJS.ProcessEnv> {
  const merged: NodeJS.ProcessEnv = { ...baseEnv };
  if (!merged['TASKSAIL_PYTHON_BASE_IMAGE']) {
    const fileEnv = await loadEnv(path.join(repoRoot, '.env'));
    const fromFile = fileEnv.get('TASKSAIL_PYTHON_BASE_IMAGE');
    if (fromFile) merged['TASKSAIL_PYTHON_BASE_IMAGE'] = fromFile;
  }
  return createSharedMcpBootstrapEnv(mcpPort, merged);
}
...
function renderSharedMcpComposeOverride(
  externalRoots: string[],
  engineOptions: EngineHostPathOptions = {},
): string {
  const lines = [
  ...
  lines.push('    volumes:');
  for (const [index, externalRoot] of externalRoots.entries()) {
    const resolvedSource = toEngineHostPath(path.resolve(externalRoot), engineOptions);
    lines.push(
      '      - type: bind',
