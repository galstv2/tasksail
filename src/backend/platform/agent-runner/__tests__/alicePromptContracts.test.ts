...
describe('Alice prompt contracts', () => {
  it('anchors the product-manager startup prompt to instructions, worktrees, and template shape', () => {
    ...
    expect(prompt).toContain('follow the active-format slice (`slice-N.*`) authoring rules and write order from the `## Product Manager Artifact Checklist` in the Runtime Path Manifest');
    expect(prompt).toContain('create and populate every planned active-format slice (`slice-N.*`)');
    ...
    expect(prompt).toContain('then continue to create and populate every planned active-format slice (`slice-N.*`), and write `parallel-ok.md` last');
  });
  ...
  it('tells Alice that active source inspection uses task worktree roots', () => {
    ...
    expect(instructions).toContain('then continue to create and populate every planned active-format slice (`slice-N.*`), and write `parallel-ok.md` last');
