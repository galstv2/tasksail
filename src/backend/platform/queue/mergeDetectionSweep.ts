...
export async function runMergeDetectionSweep(repoRoot: string): Promise<SweepResult> {
  const result: SweepResult = { scanned: 0, bindingsMarked: 0, tasksFullyMerged: 0, tasksCleanedUp: 0 };
  ...
  for (const taskId of readdirSync(tasksDir)) {
    ...
    if (sidecar.state !== 'completed') continue;
    ...
    const allHandled = bindings.every((b) => b.mergedAt !== undefined);

    if (allHandled) {
      sidecar.state = 'merged';
      result.tasksFullyMerged += 1;

      // Idempotent branch ref deletion — skip silently if absent or in use.
      for (const binding of bindings) {
        try {
          await execFileP('git', [
            '-C', binding.originalRoot,
          ...
          // Already gone or refused; next sweep will re-evaluate.
        }
      }
      ...
      try {
        ...
        // Filesystem holds; leave for next sweep. Do NOT persist 'merged'
        // state — keeping the sidecar at 'completed' with all bindings
        // stamped means the next sweep re-enters this branch idempotently.
      }
    } else if (mutated) {
