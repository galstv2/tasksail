...
def _determine_estate_mode(
    ...
    if requested_mode in {"distributed", "monolith"}:
        return requested_mode

    answer_mode = normalize_optional_string(answers.get("discovery_mode")).lower()
    if answer_mode in {"distributed", "monolith"}:
        return answer_mode

    answer_estate_type = normalize_optional_string(answers.get("estate_type")).lower()
    if answer_estate_type.startswith("distributed"):
        return "distributed"
    if answer_estate_type.startswith("monolith"):
        return "monolith"
