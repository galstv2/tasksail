...
# ---------------------------------------------------------------------------
# Path classification helpers (G4 scary path)
# ---------------------------------------------------------------------------
...
class PackPreflightValidator:
    """Aggregates G1–G6 preflight checks into a single non-short-circuiting pass."""
    ...
    def _check_paths(self) -> None:
        ...
        # itself is created by mkdir(recursive=True), so we only inspect the
        # parent.
        ctx_dir = self._request.context_pack_dir
        ctx_parent = ctx_dir.parent
        if not ctx_parent.is_dir() or not os.access(ctx_parent, os.W_OK):
            self._errors.append(PreflightError(
