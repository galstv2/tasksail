...
_TESTS_MD_REL = Path("AgentWorkSpace/handoffs/tests.md")
_LOCK_REL = Path(".platform-state/runtime/parallel/tests-md.lock")
...
def instance_section_exists(root_dir: Path, instance_id: str) -> bool:
    ...
    """
    return _instance_marker(instance_id) in load_text(root_dir / _TESTS_MD_REL)
...
def append_tests_md_section(
    ...
    (idempotency guard).
    """
    tests_md = root_dir / _TESTS_MD_REL
    lock_path = root_dir / _LOCK_REL
