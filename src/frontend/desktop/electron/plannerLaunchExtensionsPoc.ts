...
type SkillDirConfig = { path: string; discovered_skills: LilyPocSkillMetadata[] };
class PocRejection extends Error {
  constructor(
    readonly reasonCode: ReasonCode,
    readonly extra: { fieldName?: string; pathCategory?: PathCategory } = {},
  ) {
    super(`Lily launch extensions are invalid: ${reasonCode}`);
  }
}
...
async function resolveLilyPlannerLaunchExtensionsPocUnchecked(
  ...
  let parsed: unknown;
  try {
    parsed = safeJsonParse(raw, LILY_LAUNCH_EXTENSIONS_POC_CONFIG_PATH);
  } catch {
...
export async function scanExplicitLilyPocSkillPathForMetadata(
  skillPath: string,
): Promise<LilyPocSkillMetadata[]> {
  ...
  return metadata.filter((entry): entry is LilyPocSkillMetadata => entry !== undefined);
}
...
function readCachedSkillMetadata(value: unknown): LilyPocSkillMetadata {
  if (!isRecord(value) ||
...
async function readSkillMetadata(skillFilePath: string): Promise<LilyPocSkillMetadata | undefined> {
...
function emptyResult(): ResolvedLilyPlannerLaunchExtensionsPoc { return { launchExtensions: undefined, pluginDirCount: 0, skillDirCount: 0 }; }
