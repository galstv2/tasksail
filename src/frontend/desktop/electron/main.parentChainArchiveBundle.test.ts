...
describe('readParentChainArchiveBundleAction', () => {
  ...
  it('routes through the injected desktop action handler', async () => {
    ...
    const response = {
      ...
      response: {
        ...
        accepted: true,
        message: 'Loaded.',
        bundle: { schemaVersion: 1, parentTaskId: 'parent', rootTaskId: 'root', currentTipTaskId: null, status: 'no-chain-state', tasks: [], missingTaskIds: [], totalBytes: 0, truncated: false },
      },
    };
    const handler = vi.fn().mockResolvedValue(response);

    await expect(handleDesktopAction(
      { action: 'planner.readParentChainArchiveBundle', payload: payloadValue },
      { readParentChainArchiveBundle: handler },
    )).resolves.toBe(response);
    expect(handler).toHaveBeenCalledWith(payloadValue);
  });
});
