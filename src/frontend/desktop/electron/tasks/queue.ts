...
import { listAvailableContextPacks, readWorkspaceSyncStateSnapshot } from '../contextPack/catalog';
import { derivePlannerDraftTitle, derivePlannerScopeMode, isMonolithEstate } from '../planner/staging';
import { REPO_ROOT } from '../paths';
import { resolveChildTaskChainCreationContext } from '../archive/childTaskChain';
import {
...
import { deriveBypassPlannerTaskTitle } from '../planner/title';
...
async function broadcastTaskBoardAfterQueueMutation(): Promise<void> {
  const { broadcastTaskBoardUpdate } = await import('./board');
  await broadcastTaskBoardUpdate(listAvailableContextPacks);
}
...
export async function runDropboxTaskScript(options: {
  ...
  await broadcastTaskBoardAfterQueueMutation();
...
function validateUploadedSpecContent(
  ...
  if (/^#\s+\S/m.test(content)) {
    return 'Uploaded spec must not include a top-level title (# heading). Bypass Planner task titles are generated from uploaded content. Remove it and re-upload.';
  }
