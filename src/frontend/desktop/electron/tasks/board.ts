...
export async function readTaskBoard(
  ...
  try {
    ...
    // legacy markdown files appear when the registry is empty
...
// Returns artifacts sorted with the canonical archive first, then by path.
...
// Artifact selection lookup: defaults to the canonical archive when no path was requested,
...
// A requested nested path cannot be served by a flat/unreadable archive.
...
async function readCompletedTaskContent(
  ...
  if (match?.archivePath) {
    ...
    if (root) {
      try {
        ...
        // Archive root unreadable; fall back to the flat archive below.
        listed = null;
      }
    }
    ...
    // so this is always a flat archive and carries no branch-handoff source.
    const scope = await resolveActiveContextPackTaskScope(listContextPacks);
    if (!scope) {
      return notFoundResult(base);
    }
  ...
  // Branch-handoff text belongs only on the canonical archive; nested artifacts render verbatim.
