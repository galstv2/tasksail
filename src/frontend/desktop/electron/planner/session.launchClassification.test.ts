...
import { classifyPlannerLaunchAllowedRoots } from './session.launchClassification';
...
const {
  ...
  resolvePlannerLaunchExtensions: vi.fn((args: { plannerSessionId: string }) => Promise.resolve({
...
describe('planner launch allowed-root classification', () => {
  ...
  it('classifies regular distributed-platform launch roots as platform allowlist or live baseline and logs parentBranchView none', async () => {
    ...
    const classificationCall = info.mock.calls.find(
      ([eventName]) => eventName === 'planner.session.launch.allowedRoots.classification',
    );
    expect(classificationCall).toBeDefined();
    expect(Object.keys(classificationCall![1])).toEqual([
      'sessionId',
      'contextPackDir',
      'rootCount',
      'parentBranchView',
