...
vi.mock('./staging', () => ({ clearStagingArtifacts, initializeStagedPlanningDraft }));

vi.mock('./history', () => ({ appendPendingMessage, beginPendingRecord, discardPendingRecord }));
...
vi.mock('./launchExtensions', async (importOriginal) => {
  const actual = await importOriginal<typeof import('./launchExtensions')>();
  return { ...actual, resolvePlannerLaunchExtensions };
});
...
describe('plannerSession staging bootstrap', () => {
  beforeEach(() => {
    vi.doUnmock('./cliProcess');
    vi.doUnmock('./parentBranchView');
    vi.resetModules();
    // Vitest 4: resetAllMocks (not clearAllMocks) also flushes leftover
    // *Once queues between tests, which restoreMocks no longer does.
    vi.resetAllMocks();
    ...
    resolvePlannerLaunchExtensions.mockResolvedValue(resolvedPlanner());
  });
  ...
  it('does not re-resolve planner extensions or recreate a stage when the session is reused', async () => {
    resolveSelectedPrimaryRepoRoot.mockResolvedValue(undefined);
    ...
    resolvePlannerLaunchExtensions.mockRejectedValueOnce(new Error('should not re-read active session config'));
    ...
    expect(resolvePlannerLaunchExtensions).toHaveBeenCalledTimes(1);
  });
  ...
  it('passes staged launchExtensions to broker.startSession with plannerSessionId as the launch ID', async () => {
    ...
    resolvePlannerLaunchExtensions.mockResolvedValueOnce(resolvedPlanner({ launchExtensions }));

    const { PlannerSessionBroker } = await import('./sessionBroker');
  ...
  it('generates distinct monotonic plannerSessionIds (and stage launch IDs) for same-millisecond sessions', async () => {
    ...
    const calls = resolvePlannerLaunchExtensions.mock.calls;
  ...
  it('rejects invalid planner assignments before broker.startSession, staging, and provider spawn', async () => {
    resolvePlannerLaunchExtensions.mockRejectedValueOnce(new Error('planner extensions unavailable.'));
    const { PlannerSessionBroker } = await import('./sessionBroker');
    const startSession = vi.spyOn(PlannerSessionBroker.prototype, 'startSession');

    const plannerSession = await import('./session');
    await expect(plannerSession.startSession('/contextpacks/test')).rejects.toThrow(
      'planner extensions unavailable.',
    );
  ...
  it('resolves planner launch extensions after reasoning validation and before branch-view work', async () => {
    const order: string[] = [];
    vi.doMock('./parentBranchView', () => ({
    ...
    resolvePlannerLaunchExtensions.mockImplementationOnce(() => {
  ...
  it('cleans the planner extension stage when broker.startSession throws', async () => {
    resolveSelectedPrimaryRepoRoot.mockResolvedValue(undefined);
    resolveFocusedRepoRoot.mockResolvedValue(undefined);
    const cleanup = vi.fn().mockResolvedValue(undefined);
    resolvePlannerLaunchExtensions.mockResolvedValueOnce(resolvedPlanner({ cleanup }));
    const { PlannerSessionBroker } = await import('./sessionBroker');
  ...
  it('cleans the newly created planner extension stage when broker returns created false', async () => {
    resolveSelectedPrimaryRepoRoot.mockResolvedValue(undefined);
    resolveFocusedRepoRoot.mockResolvedValue(undefined);
    const cleanup = vi.fn().mockResolvedValue(undefined);
    resolvePlannerLaunchExtensions.mockResolvedValueOnce(resolvedPlanner({ cleanup }));
  ...
  it('cleans the planner extension stage when parent branch view creation fails after staging', async () => {
    resolveSelectedPrimaryRepoRoot.mockResolvedValue(undefined);
    resolveFocusedRepoRoot.mockResolvedValue(undefined);
    const cleanup = vi.fn().mockResolvedValue(undefined);
    resolvePlannerLaunchExtensions.mockResolvedValueOnce(resolvedPlanner({ cleanup }));
  ...
  it('ends the failed broker session and cleans stale handles before resolving a replacement snapshot', async () => {
    ...
    resolvePlannerLaunchExtensions
      .mockResolvedValueOnce(resolvedPlanner({ cleanup: cleanupA }))
      .mockResolvedValueOnce(resolvedPlanner({ cleanup: cleanupB }));
    ...
    expect(resolvePlannerLaunchExtensions).toHaveBeenCalledTimes(2); // replacement resolves a fresh snapshot
  });

  it('cleans stale planner extension and parent-branch-view handles on a failed-broker replacement even when reasoning validation throws', async () => {
    resolveSelectedPrimaryRepoRoot.mockResolvedValue(undefined);
    resolveFocusedRepoRoot.mockResolvedValue(undefined);
    initializeStagedPlanningDraft.mockResolvedValue(undefined);
    clearStagingArtifacts.mockResolvedValue(undefined);
    ...
    vi.doMock('./cliProcess', async (importOriginal) => ({
      ...(await importOriginal<typeof import('./cliProcess')>()),
    ...
    const cleanupA = vi.fn().mockResolvedValue(undefined);
    resolvePlannerLaunchExtensions.mockResolvedValueOnce(resolvedPlanner({ cleanup: cleanupA }));
  ...
  it('cleans up owned staging and the planner extension stage when the planner session ends', async () => {
    resolveSelectedPrimaryRepoRoot.mockResolvedValue(undefined);
    resolveFocusedRepoRoot.mockResolvedValue(undefined);
    initializeStagedPlanningDraft.mockResolvedValue(undefined);
    clearStagingArtifacts.mockResolvedValue(undefined);
  ...
  it('continues staging cleanup on endSession even when the planner extension cleanup handle reports a failure', async () => {
    resolveSelectedPrimaryRepoRoot.mockResolvedValue(undefined);
    resolveFocusedRepoRoot.mockResolvedValue(undefined);
    initializeStagedPlanningDraft.mockResolvedValue(undefined);
    clearStagingArtifacts.mockResolvedValue(undefined);
  ...
  it('logs staging bootstrap failures, ends the broker session, and cleans the planner extension stage', async () => {
    resolveSelectedPrimaryRepoRoot.mockResolvedValue(undefined);
    resolveFocusedRepoRoot.mockResolvedValue(undefined);
    clearStagingArtifacts.mockResolvedValue(undefined);
  ...
  it('prepends runtime launch notes only to the first planner turn and stores only the Guide display text', async () => {
    resolveSelectedPrimaryRepoRoot.mockResolvedValue(undefined);
    resolveFocusedRepoRoot.mockResolvedValue(undefined);
    initializeStagedPlanningDraft.mockResolvedValue(undefined);
    clearStagingArtifacts.mockResolvedValue(undefined);
    resolvePlannerLaunchExtensions.mockResolvedValueOnce(
      resolvedPlanner({ availabilityNote: 'NOTE-BODY' }),
    );
    const { PlannerSessionBroker } = await import('./sessionBroker');
    ...
    expect(firstSent).toContain('--- ACTIVE PLANNER TASK SUBJECT BOUNDARY ---');
    expect(firstSent).toContain('TaskSail is workflow protocol only for this planner session.');
    expect(firstSent).toContain('Active context pack dir: /contextpacks/test');
    expect(firstSent).toContain('Resolved target repo roots: none');
    ...
    expect(JSON.stringify(appendPendingMessage.mock.calls)).not.toContain('ACTIVE PLANNER TASK SUBJECT BOUNDARY');
  });

  it('includes resolved context-pack focus details in the first-turn task-subject boundary', async () => {
    resolveSelectedPrimaryRepoRoot.mockResolvedValue(undefined);
    resolveFocusedRepoRoot.mockResolvedValue({
      primaryRepoRoot: '/repos/backend',
      visibleRepoRoots: ['/repos/backend', '/repos/tools'],
      declaredRepoRoots: ['/repos/backend', '/repos/tools'],
      estateType: 'distributed-platform',
      primaryRepoId: 'backend',
      primaryFocusId: 'api',
      ...
      primaryFocusTargets: [
        { path: 'services/api', kind: 'directory', role: 'anchor' },
        { path: 'services/worker', kind: 'directory', role: 'primary' },
      ],
      writableRoots: [
        { path: 'services/api', kind: 'directory', reason: 'selected-primary' },
      ],
      readonlyContextRoots: [
        { path: 'tools/docs', kind: 'directory', reason: 'support-target' },
      ],
      selectedRepoIds: ['backend', 'tools'],
      selectedFocusIds: ['api', 'tools'],
      authoritySource: 'manifest-primary',
    });
    initializeStagedPlanningDraft.mockResolvedValue(undefined);
    clearStagingArtifacts.mockResolvedValue(undefined);
    ...
    await plannerSession.sendMessage('What is the next best task to work on?');

    const firstSent = sendMessage.mock.calls[0][0];
    expect(firstSent).toContain('Active context pack ID: orders');
    expect(firstSent).toContain('Target repo roots:\n- /repos/backend\n- /repos/tools');
    expect(firstSent).toContain('Primary repo ID: backend');
    expect(firstSent).toContain('Primary repo root: /repos/backend');
    expect(firstSent).toContain('Selected repo IDs: backend, tools');
    expect(firstSent).toContain('Selected focus IDs: api, tools');
    expect(firstSent).toContain('Primary focus ID: api');
    expect(firstSent).toContain('Primary focus path: services/api');
    expect(firstSent).toContain('Deep Focus primary targets:\n- directory:services/api\n- directory:services/worker');
    expect(firstSent).toContain('Writable roots:\n- directory:services/api');
    expect(firstSent).toContain('Read-only context roots:\n- directory:tools/docs');
    expect(firstSent).toContain('inspect and recommend only within these context-pack roots and selected focus areas');
  });

  it('includes standard distributed repo-selection details in the first-turn task-subject boundary', async () => {
    resolveSelectedPrimaryRepoRoot.mockResolvedValue(undefined);
    resolveFocusedRepoRoot.mockResolvedValue({
      primaryRepoRoot: '/repos/orders-api',
      visibleRepoRoots: ['/repos/orders-api', '/repos/shared-tools'],
      declaredRepoRoots: ['/repos/orders-api', '/repos/shared-tools'],
      estateType: 'distributed-platform',
      primaryRepoId: 'orders-api',
      deepFocusEnabled: false,
      selectedRepoIds: ['orders-api', 'shared-tools'],
      selectedFocusIds: [],
      authoritySource: 'manifest-primary',
    });
    initializeStagedPlanningDraft.mockResolvedValue(undefined);
    ...
    const plannerSession = await import('./session');
    await plannerSession.startSession('/contextpacks/orders-distributed');
    await plannerSession.sendMessage('What is the next best task to work on?');

    const firstSent = sendMessage.mock.calls[0][0];
    expect(firstSent).toContain('Active context pack ID: orders-distributed');
    expect(firstSent).toContain('Target repo roots:\n- /repos/orders-api\n- /repos/shared-tools');
    expect(firstSent).toContain('Primary repo ID: orders-api');
    expect(firstSent).toContain('Selected repo IDs: orders-api, shared-tools');
    expect(firstSent).toContain('Selected focus IDs: none');
    expect(firstSent).toContain('Deep Focus enabled: false');
  });

  it('includes standard monolith focus-selection details in the first-turn task-subject boundary', async () => {
    resolveSelectedPrimaryRepoRoot.mockResolvedValue(undefined);
    resolveFocusedRepoRoot.mockResolvedValue({
      primaryRepoRoot: '/repos/crud-monolith',
      visibleRepoRoots: ['/repos/crud-monolith'],
      declaredRepoRoots: ['/repos/crud-monolith'],
      estateType: 'monolith',
      primaryRepoId: 'crud-monolith',
    ...
    const plannerSession = await import('./session');
    await plannerSession.startSession('/contextpacks/crud-monolith');
    await plannerSession.sendMessage('What is the next best task to work on?');

    const firstSent = sendMessage.mock.calls[0][0];
    expect(firstSent).toContain('Active context pack ID: crud-monolith');
    expect(firstSent).toContain('Target repo roots:\n- /repos/crud-monolith');
    expect(firstSent).toContain('Primary repo ID: crud-monolith');
    expect(firstSent).toContain('Selected repo IDs: none');
    expect(firstSent).toContain('Selected focus IDs: platform, tools');
    expect(firstSent).toContain('Primary focus ID: platform');
    expect(firstSent).toContain('Primary focus path: platform');
    expect(firstSent).toContain('Deep Focus enabled: false');
  });
