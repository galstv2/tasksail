// Lily launch confirmation (positive and negative paths).
//
// Drives resolvePlannerLaunchExtensions against the REAL backend staging path with a
// temp-repo fixture: production assignment + staging helpers only, never the POC
...
import { resolvePlannerLaunchExtensions } from './launchExtensions';
...
describe('resolvePlannerLaunchExtensions — lily-positive', () => {
  it('stages only planning-agent assignments and builds a metadata-only first-turn note', async () => {
    ...
    const res = await resolvePlannerLaunchExtensions({
      repoRoot: f.repo,
...
describe('resolvePlannerLaunchExtensions — lily-negative', () => {
