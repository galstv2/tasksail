...
// Launch-extension integration confirmation:
// resolver output reaches broker capture, first-turn notes stay scoped, session
// reuse skips re-resolution, and staged paths never leak into broker-facing state.
...
describe('plannerSession phase2 launch-extension confirmation', () => {
  beforeEach(() => {
    ...
    vi.resetModules();
    // Vitest 4: resetAllMocks also flushes leftover *Once queues between tests.
    vi.resetAllMocks();
  ...
  describe('lily-positive: assignments present', () => {
    it('passes staged launchExtensions to broker.startSession', async () => {
      ...
      resolvePlannerLaunchExtensions.mockResolvedValueOnce(
        resolvedPlanner({ launchExtensions }),
      );
    ...
    it('applies the resolver availability note to the first broker turn only', async () => {
      ...
      resolvePlannerLaunchExtensions.mockResolvedValueOnce(
        resolvedPlanner({ availabilityNote: note }),
      );
  ...
  describe('session reuse', () => {
    it('does not re-resolve Lily extensions for a second startSession call on the same active session', async () => {
      ...
      resolvePlannerLaunchExtensions.mockRejectedValueOnce(
    ...
    it('re-resolves extensions for a new session created after the prior session ends', async () => {
      ...
      resolvePlannerLaunchExtensions
        ...
        .mockResolvedValueOnce(resolvedPlanner());
      ...
      expect(resolvePlannerLaunchExtensions).toHaveBeenCalledTimes(2);
  ...
  it('plannerSession.ts does NOT import the Phase 1 POC resolver (plannerLaunchExtensionsPoc)', async () => {
    ...
    // the legacy POC module. The path is resolved relative to this test file's directory.
    const { readFileSync } = await import('node:fs');
    const { fileURLToPath } = await import('node:url');
    const { resolve, dirname } = await import('node:path');
    const thisDir = dirname(fileURLToPath(import.meta.url));
    const src = readFileSync(resolve(thisDir, 'session.ts'), 'utf-8');
