...
describe('planner staging helpers', () => {
  ...
  it('derives deterministic planner titles from repo and focus metadata', async () => {
    const { derivePlannerDraftTitle } = await import('./staging');
  ...
  it('suppresses Primary Repo ID for monolith packs and surfaces only Primary Focus ID', async () => {
    const { initializeStagedPlanningDraft, readOwnedStagedDraft } = await import('./staging');
  ...
  it('preserves multiple standard monolith primary focus selections without singular primary focus metadata', async () => {
    const { initializeStagedPlanningDraft, readOwnedStagedDraft, readPlannerStagingSidecar } = await import('./staging');
  ...
  it('preserves supplied staging roles and fills missing selected ids from the manifest', async () => {
    const { initializeStagedPlanningDraft, readPlannerStagingSidecar } = await import('./staging');
  ...
  it('persists Deep Focus staging metadata for planner-originated drafts', async () => {
    ...
    const { draft } = await (await import('./staging')).readOwnedStagedDraft('planner-109');
  ...
  it('reports an empty owned draft without losing metadata context', async () => {
    ...
    await expect(readOwnedStagedDraft('planner-152')).resolves.toEqual({
      draft: null,
      error: `Staged draft ${metadata.draftFilename} is empty. Ask the planner to rewrite the draft before finalizing.`,
      metadata: expect.objectContaining({
        sessionId: 'planner-152',
  ...
  it('does not reclaim the planner staging lock when the holder process is alive', async () => {
    const { acquirePlannerStagingLock } = await import('./staging');
  ...
  it('falls back to legacy newest-file reads when no sidecar exists', async () => {
    const { readStagedDraft } = await import('./staging');
    const stagingDir = join(TEST_REPO_ROOT, 'AgentWorkSpace', 'dropbox', '.staging');
  ...
  it('logs a structured rollback error when staging cleanup fails after a write failure', async () => {
    ...
    const { initializeStagedPlanningDraft } = await import('./staging');
    const contextPackDir = await writeContextPackManifest('orders', {
      repositories: [{ repo_id: 'backend', repository_type: 'primary' }],
      primary_working_repo_ids: ['backend'],
    });
