...
import { PlannerEventParser } from './eventParser';
import {
...
export class PlannerSessionBroker {
  ...
  startSession(options?: {
    ...
    this.session = {
      ...
      plannerPersonalityId: options?.plannerPersonalityId ?? 'balanced',
  ...
  updateSessionPersonality(plannerPersonalityId: PlannerPersonalityId): PlannerPersonalityUpdateResult {
    ...
    session.plannerPersonalityId = plannerPersonalityId;
    return 'updated';
  }
  ...
  private async runTurn(session: PlannerSessionRecord, text: string): Promise<boolean> {
    ...
    const child = this.spawnCliProcess({
      ...
      plannerPersonalityId: session.plannerPersonalityId,
