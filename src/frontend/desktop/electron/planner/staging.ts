...
export async function initializeStagedPlanningDraft(
  ...
  if (!title) {
    throw new Error('Planner staging requires a deterministic derived title before the planner can draft.');
  }
...
export async function readStagedDraft(sessionId?: string | null): Promise<StagedDraftReadResult> {
  ...
  try {
    ...
    if (content.trim().length === 0) {
      return {
        draft: null,
        error: `Staged draft ${newest.name} is empty. Ask the planner to rewrite the draft before finalizing.`,
      };
    }
