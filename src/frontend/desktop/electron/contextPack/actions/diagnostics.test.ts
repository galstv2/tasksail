...
// create.ts gates contextPackDir to <REPO_ROOT>/contextpacks. Each create test
// makes its temp `root` BE that contextpacks dir and points REPO_ROOT at the parent,
// so the ownership guard passes while contextPackDir's parent stays `root`.
...
describe('context-pack action diagnostics', () => {
  ...
  it('logs malformed bootstrap JSON during context-pack creation', async () => {
    const repoRoot = await mkdtemp(join(tmpdir(), 'context-pack-create-diagnostics-'));
    const root = join(repoRoot, 'contextpacks');
    try {
      await mkdir(root, { recursive: true });
      pathsMock.repoRoot = repoRoot;
  ...
  it('logs preflight rejection diagnostics during context-pack creation', async () => {
    const repoRoot = await mkdtemp(join(tmpdir(), 'context-pack-create-preflight-diagnostics-'));
    const root = join(repoRoot, 'contextpacks');
    try {
      await mkdir(root, { recursive: true });
      pathsMock.repoRoot = repoRoot;
