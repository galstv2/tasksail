...
import { filterActiveTaskIdsForScope, type ActiveContextPackTaskScope } from '../taskVisibility';
...
function assertTaskSailOwnedDeletePath(pathToDelete: string, ownerRoot: string, label: string): void {
  const resolvedPath = resolve(pathToDelete);
  const resolvedOwnerRoot = resolve(ownerRoot);
  if (resolvedPath === resolvedOwnerRoot || !isPathInside(resolvedPath, resolvedOwnerRoot)) {
    throw new Error(`Refusing to delete non-TaskSail-managed ${label} path: ${pathToDelete}`);
  }
}

async function hasGitMarker(dir: string): Promise<boolean> {
  try {
    const marker = await stat(join(dir, '.git'));
...
export async function executeContextPackDeleteAction(
  ...
  try {
    ...
    assertTaskSailOwnedDeletePath(entry.contextPackDir, join(REPO_ROOT, 'contextpacks'), 'context pack');
    assertTaskSailOwnedDeletePath(mirrorDir, join(REPO_ROOT, 'AgentWorkSpace', 'qmd', 'context-packs'), 'context pack mirror');
    if (await hasGitMarker(entry.contextPackDir)) {
      throw new Error(`Refusing to delete context pack path because it is a Git repository root: ${entry.contextPackDir}`);
    }
