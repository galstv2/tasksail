...
export async function runPythonScriptCommand(
  ...
  return new Promise((resolve, reject) => {
    ...
    let stderr = '';
    let settled = false;
    const failWithOutput = (code: number | null, signal: NodeJS.Signals | null): Error => Object.assign(
      new Error(
        stderr.trim()
          || stdout.trim()
          || `Python script exited with ${typeof code === 'number' ? `code ${code}` : `signal ${signal ?? 'unknown'}`}.`,
      ),
      { stdout, stderr, exitCode: code, signalCode: signal },
    );
    ...
    child.stdin.end();
    child.on('close', (code: number | null, signal: NodeJS.Signals | null) => settle(() => {
      if ((typeof code === 'number' && code !== 0) || signal) {
        reject(failWithOutput(code, signal));
        return;
      }
