...
describe('contextPack delete action', () => {
  ...
  it('refuses to delete external catalog paths outside TaskSail contextpacks', async () => {
    const { repoRoot, module } = await loadModule();
    const sourceRepoRoot = join(repoRoot, 'external-source-repo');
    await mkdir(sourceRepoRoot, { recursive: true });

    const result = await module.executeContextPackDeleteAction(
      { contextPackDir: sourceRepoRoot },
      async () => ({
        action: 'contextPack.list',
        mode: 'read-only',
        message: '1 pack.',
        ...
        searchRoots: [],
        recentContextPackDirs: [],
        contextPacks: [catalogEntry(sourceRepoRoot, false)],
      }),
    );

    expect(result.ok).toBe(false);
    expect(result.ok ? null : result.error).toContain('non-TaskSail-managed context pack path');
    expect(await exists(sourceRepoRoot)).toBe(true);
  });

  it('refuses to delete a Git repository root inside TaskSail contextpacks', async () => {
    const { repoRoot, module } = await loadModule();
    const packDir = join(repoRoot, 'contextpacks', 'pack-a');
    await mkdir(join(packDir, '.git'), { recursive: true });
    ...
    expect(result.ok).toBe(false);
    expect(result.ok ? null : result.error).toContain('Git repository root');
    expect(await exists(packDir)).toBe(true);
  });
