...
import { validateChildTaskFocusSnapshot } from '../planner/focusValidation';
import {
  hydrateConversationAction,
  listConversationHistoryAction,
} from '../planner/history';
...
import { listArchivedTasksAction } from '../archive/archivedTasks';
import { readParentContextBundleAction } from '../archive/parentContextBundle';
import { readParentChainArchiveBundleAction } from '../archive/parentChainArchiveBundle';
import { readParentArchiveMarkdownAction } from '../archive/parentArchiveMarkdown';
import { readChildTaskChainBranchInventoryAction } from '../archive/childTaskChainBranchInventory';
import {
  readTaskBoard,
...
export function createDefaultDesktopActionHandlers(
  ...
  startPlannerSession: (payload) => {
    ...
    return plannerSession.startSession(
      ...
      payload?.plannerPlanningReloadScope,
      payload?.parentTaskBranchView,
      payload?.plannerPersonalityId,
    );
  },
  updatePlannerSessionPersonality: (payload) => plannerSession.updateSessionPersonality(payload.plannerPersonalityId),
