...
import { emitStreamEvent } from '../runtime/stream';
import { pathExists, repoFs } from '../utils';
import { createLogger } from '../log/logger';

const log = createLogger('electron/main.recovery');

// Per-task runtime paths. Singleton constants are intentionally absent.
// Active markers live in ACTIVE_ITEMS_DIR, one file per taskId.
...
/** queueName of the first active marker, or null. */
...
/** mtime of the first active marker file, or null. */
...
/**
 * hasPipelineStartEvidence is per-task.
 * Checks per-task runtime dir at .platform-state/runtime/tasks/<taskId>/.
 * taskId is derived from activeQueueName after removing the queue-file extension.
 */
async function hasPipelineStartEvidence(
  ...
  // Per-task pipeline lock; there is no singleton PIPELINE_LOCK_DIR.
...
/**
 * Switch on .kind (structured) instead of matching rendered strings.
...
/**
 * Queue divergence = pending items without active markers,
...
export function startTaskRecoveryController(options: {
  ...
  const scheduleAutoStartForNext = async (queueName: string | null): Promise<void> => {
    ...
    // Guard per-task lock instead of singleton PIPELINE_LOCK_DIR.
  ...
  const reconcile = async (): Promise<void> => {
    ...
    try {
      ...
      if (!activeQueueName) {
        // Use structuredIssues instead of string-matched issues array.
      ...
      // Use structuredIssues for fixable issue detection.
      ...
      if (hasRuntimeEvidence) {
        ...
        // Check per-task lock instead of singleton PIPELINE_LOCK_DIR.
