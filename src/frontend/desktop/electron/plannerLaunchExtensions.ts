...
export type ResolveLilyPlannerLaunchExtensionsOptions = {
  repoRoot: string;
  plannerSessionId: string;
...
export type LilyLaunchAvailabilityNoteEntry = {
  id: string;
  kind: 'skill' | 'plugin';
  displayName: string;
  description: string;
...
export type LilyLaunchPluginComponentSummary = {
  pluginId: string;
...
export const LILY_EXTENSIONS_UNAVAILABLE_MESSAGE =
  'Lily did not start because one or more assigned skills or plugins are unavailable. ' +
...
const NOTE_INTRO =
  'The following extensions are available for this Lily planning session. ' +
...
export function buildLilyLaunchAvailabilityNote(
  entries: readonly LilyLaunchAvailabilityNoteEntry[],
): string | undefined {
  if (entries.length === 0) {
    return undefined;
  }
...
export function applyLilyLaunchAvailabilityNoteToFirstTurn(args: {
  guideText: string;
...
export async function resolveLilyPlannerLaunchExtensions(
  ...
  try {
    ...
    throw new Error(LILY_EXTENSIONS_UNAVAILABLE_MESSAGE);
  }
  ...
  const pluginComponents: LilyLaunchPluginComponentSummary[] = entries
    .filter((entry) => entry.kind === 'plugin')
    .map((entry) => ({ pluginId: entry.id, bundledSkillCount: entry.metadata.plugin_skill_count ?? 0 }));

  const noteEntries: LilyLaunchAvailabilityNoteEntry[] = entries.map((entry) => ({
    id: entry.id,
    kind: entry.kind,
  ...
  const availabilityNote = buildLilyLaunchAvailabilityNote(noteEntries);
