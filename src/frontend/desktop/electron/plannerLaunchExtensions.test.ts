...
import {
  ...
  buildLilyLaunchAvailabilityNote,
  LILY_EXTENSIONS_UNAVAILABLE_MESSAGE,
...
describe('resolveLilyPlannerLaunchExtensions', () => {
  it('returns no launchExtensions and no availabilityNote when Lily has no enabled assignments', async () => {
    ...
    const resolved = await resolveLilyPlannerLaunchExtensions({
      repoRoot: '/repo',
  ...
  it('calls createAgentExtensionStage with agentId planning-agent and launchId plannerSessionId', async () => {
    ...
    await resolveLilyPlannerLaunchExtensions({
      repoRoot: '/repo',
  ...
  it('builds a note with only display names, Skill or Plugin kind, cached descriptions, and cached plugin bundled skill names', async () => {
    ...
    const { availabilityNote } = await resolveLilyPlannerLaunchExtensions({
      repoRoot: '/repo',
      plannerSessionId: 'planner-1',
      providerId: 'copilot',
    });
  ...
  it('fails closed and logs resolve.failed when an enabled assigned entry lacks cached display metadata', async () => {
    ...
    ).rejects.toThrow(LILY_EXTENSIONS_UNAVAILABLE_MESSAGE);
...
describe('buildLilyLaunchAvailabilityNote', () => {
  it('returns undefined for an empty entry list', () => {
    expect(buildLilyLaunchAvailabilityNote([])).toBeUndefined();
  });
});

describe('applyLilyLaunchAvailabilityNoteToFirstTurn', () => {
  it('prepends the note then applies the fresh-session wrap', () => {
    const result = applyLilyLaunchAvailabilityNoteToFirstTurn({
      guideText: 'GUIDE',
