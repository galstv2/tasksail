...
describe('plannerChildScope reload scope postcondition', () => {
  it('passes for distributed-platform standard unions', () => {
    ...
    expect(() => __plannerChildScopeTestHooks.assertLilyPlanningReloadScopePostcondition(
      parent,
      child,
      buildLilyPlanningReloadScope(parent, child),
    )).not.toThrow();
  });
  ...
  it('detects drop, duplicate, wrong-role, wrong-purpose, and mismatched-context-pack divergences', () => {
    ...
    const output = buildLilyPlanningReloadScope(parent, child);

    expect(() => __plannerChildScopeTestHooks.assertLilyPlanningReloadScopePostcondition(parent, child, {
      ...output,
      selectedRepoIds: ['tools', 'docs'],
    ...
    expect(() => __plannerChildScopeTestHooks.assertLilyPlanningReloadScopePostcondition(parent, child, {
      ...output,
      purpose: 'wrong' as 'lily-planning-read-context',
    })).toThrow(/purpose/);
    expect(() => __plannerChildScopeTestHooks.assertLilyPlanningReloadScopePostcondition(parent, child, {
      ...output,
      contextPackId: 'other',
