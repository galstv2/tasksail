...
function createClient(overrides?: Partial<DesktopShellClient>): DesktopShellClient {
  return createMockClient({
    describeActiveProvider: vi.fn().mockResolvedValue(createProviderFrontendDescriptor({
      ...
      roster: [
        { agentId: 'planning-agent', roleName: 'Planning Specialist', humanName: 'Lily', workflowOrder: 1, roleKind: 'planner' },
      ],
    })),
  ...
  it('upload-review finalize surfaces intake validation errors as draftError', async () => {
    const client = createClient({
      ...
      finalizeSpec: vi.fn().mockResolvedValue({
        ...
        error: 'Staged draft is missing required section content: Desired Outcome. Ask the planner to complete the planning intake before finalizing.',
      }),
    });
    const { result } = renderPlannerModalHook(client);
    ...
    expect(startPlannerSession).toHaveBeenLastCalledWith({
      ...
      parentTaskBranchView: {
        ...
        branchChainAvailability: {
          ...
          message: 'Parent branch view unavailable: archived parent has no branch handoffs. The planner will use archived parent archive context only.',
        },
      },
    });
  });
