...
import { FALLBACK_PLANNER_DISPLAY, getPlannerDisplayModel, type PlannerDisplayModel } from '../../../shared/agentRoster';
...
import type { PlannerModalProps, PlannerSessionStatus } from '../../components/planner/PlannerModal';
import { buildRecentTaskScopeSummary, type PlannerWorkspaceScopeSummary } from '../../planner/plannerWorkspaceScope';
import {
  createFollowUpDraft,
  createLocalDraft,
...
} from '../../planner/plannerChildScopeSession';
import { buildChildTaskLineage } from '../../planner/plannerArchivedTaskHelpers';
...
function chooseBusyBadgeLabel(personalityId: PlannerPersonalityId): BusyBadgeLabel {
...
export function usePlannerModal(
  ...
  const [plannerDisplay, setPlannerDisplay] = useState<PlannerDisplayModel>(FALLBACK_PLANNER_DISPLAY);
  const plannerDisplayRef = useRef<PlannerDisplayModel>(FALLBACK_PLANNER_DISPLAY);
  const [selectedPlannerPersonalityId, setSelectedPlannerPersonalityId] = useState<PlannerPersonalityId>('balanced');
  const selectedPlannerPersonalityIdRef = useRef<PlannerPersonalityId>('balanced');
  ...
  useEffect(() => {
    ...
    client.describeActiveProvider()
      .then((descriptor) => {
        ...
        const display = getPlannerDisplayModel(descriptor);
        plannerDisplayRef.current = display;
      ...
      .catch((error: unknown) => {
        ...
        plannerDisplayRef.current = FALLBACK_PLANNER_DISPLAY;
        setPlannerDisplay(FALLBACK_PLANNER_DISPLAY);
        log.warn('planner.provider_descriptor.load.failed', {
  ...
  const startSession = useCallback(() => {
    ...
    client.startPlannerSession({
      ...
      plannerPersonalityId: selectedPlannerPersonalityIdRef.current,
  ...
  const openPlannerModal = useCallback(() => {
    ...
    selectedPlannerPersonalityIdRef.current = 'balanced';
    setSelectedPlannerPersonalityId('balanced');
  ...
  const handlePlannerPersonalityChange = useCallback((id: PlannerPersonalityId): void => {
    if (personalityLocked) {
      return;
    }
    const previousId = selectedPlannerPersonalityIdRef.current;
    selectedPlannerPersonalityIdRef.current = id;
    setSelectedPlannerPersonalityId(id);
    void client.updatePlannerSessionPersonality({ plannerPersonalityId: id })
      .then((result) => {
        if (!result.ok) {
          selectedPlannerPersonalityIdRef.current = previousId;
          setSelectedPlannerPersonalityId(previousId);
        }
      })
      .catch(() => {
        selectedPlannerPersonalityIdRef.current = previousId;
  ...
  // Hold child-task context until the operator's first message so staged draft
  // context and operator intent arrive together.
  ...
  const saveChildScopeOverride = useCallback(async (args: SaveChildScopeArgs): Promise<boolean> => {
    ...
    try {
      ...
      plannerStream.sendMessage(`[Child scope adjusted — ${plannerDisplayRef.current.plannerName} reloaded with updated implementation scope and read-only parent context.]`);
      return true;
    } catch (error: unknown) {
  ...
  const handleSelectParentTask = useCallback((task: ArchivedTaskEntry) => {
    ...
    void (async () => {
      try {
        ...
        // Build with the selected parent content now, but send only with the
        // first operator message to avoid an eager "thinking" transition.
  ...
  const handleSelectConversation = useCallback((recordId: string): void => {
    ...
    void (async () => {
      try {
        ...
        // Recent replays surface scope from the hydrated sidecar binding.
        ...
        // Recent replay always starts a fresh standalone draft; the source chain
        // remains untouched.
        setChildTaskMode(false);
        setSelectedParentTask(null);
        setDraft(createLocalDraft(EMPTY_DRAFT_SEED));
        ...
        if (start.response.message.toLowerCase().includes('focus') && start.response.message.toLowerCase().includes('resolved')) {
          setDraftError(`Some referenced focus paths could not be resolved. ${plannerDisplayRef.current.plannerName} will use the saved variables you provided.`);
        }
      } catch (error: unknown) {
        expectedSessionIdRef.current = null;
  ...
  const handleSendMessage = useCallback(
    (text: string): void => {
      void (async () => {
        ...
        setBusyBadgeLabel(chooseBusyBadgeLabel(selectedPlannerPersonalityIdRef.current));
        ...
        // Send deferred parent context with the operator's first turn.
      ...
      try {
        ...
        for (let attempt = 0; attempt < DRAFT_READ_MAX_ATTEMPTS; attempt += 1) {
          ...
          if (response.brokerStatus !== 'running') {
            setDraftError(`${plannerDisplayRef.current.plannerName} has not written a draft yet. Try again shortly.`);
            return;
          }
        ...
        setDraftError(`${plannerDisplayRef.current.plannerName} is still writing the draft. Try again shortly.`);
      } catch (error: unknown) {
        if (isStale()) return;
  ...
  const plannerModalProps = useMemo(
    () => ({
      ...
      // Recent replay shows saved scope; live child-task mode uses parent/adjusted affordances.
      ...
      sessionStatus,
      plannerDisplay,
      plannerPersonalityId: selectedPlannerPersonalityId,
      personalityLocked,
      onPlannerPersonalityChange: handlePlannerPersonalityChange,
      busyBadgeLabel,
