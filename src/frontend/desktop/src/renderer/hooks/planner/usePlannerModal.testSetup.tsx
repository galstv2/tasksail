...
const DEFAULT_PLANNER_NAME = ['Li', 'ly'].join('');
const DEFAULT_PROVIDER_ID = ['co', 'pilot'].join('');
const DEFAULT_PLANNER_AGENT_ID = ['planning', 'agent'].join('-');
...
export function createClient(overrides?: Partial<DesktopShellClient>): DesktopShellClient {
  return createMockClient({
    describeActiveProvider: vi.fn().mockResolvedValue(createProviderFrontendDescriptor({
      ...
      plannerAgentId: DEFAULT_PLANNER_AGENT_ID,
      roster: [
        { agentId: DEFAULT_PLANNER_AGENT_ID, roleName: 'Planning Specialist', humanName: DEFAULT_PLANNER_NAME, workflowOrder: 1, roleKind: 'planner' },
      ],
    })),
