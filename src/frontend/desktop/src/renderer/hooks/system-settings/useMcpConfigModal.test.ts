...
describe('useMcpConfigModal — integration round-trips', () => {
  ...
  // Env-ref header drafts must reach an operator-visible failed
  // state, not a silent block. Save must be disabled and the message must be
  // visible through connectionValidation (not undefined or empty).
  it('RG-04-savegate: env-ref header draft reaches operator-visible failed state, not silent block', async () => {
    const client = createMockClient({
      validateExternalMcpConnection: vi.fn().mockResolvedValue({
        ok: true,
        response: {
          ...
          mode: 'validated',
          success: false,
          // Simulates the handler refusing to resolve env-refs for unsaved draft.
          message: 'Headers use environment-variable references and cannot be validated before the server is saved. Save the server first, then re-open it to validate the live connection.',
        },
      }),
    });
    const { result } = renderHook(() => useMcpConfigModal(client));
    ...
    act(() => { result.current.mcpConfigModalProps.onAdd(); });
    act(() => {
      result.current.mcpConfigModalProps.onDraftChange('display_name', 'Env Ref MCP');
      result.current.mcpConfigModalProps.onDraftChange('purpose', VALID_PURPOSE);
      result.current.mcpConfigModalProps.onDraftChange('preferred_for', VALID_PREFERRED_FOR);
      result.current.mcpConfigModalProps.onDraftChange('url', 'https://mcp.example.com/sse');
      result.current.mcpConfigModalProps.onDraftChange('headers', [{ key: 'Authorization', value: '${API_SECRET}' }]);
    });

    await act(async () => { await result.current.mcpConfigModalProps.onValidateConnection(); });

    // State is 'failed' (operator-visible), not 'idle' or 'success'.
    expect(result.current.mcpConfigModalProps.connectionValidation.status).toBe('failed');
    // The message must be non-empty and not contain the variable name (no enumeration oracle).
    const state = result.current.mcpConfigModalProps.connectionValidation;
    expect(state.status === 'failed' && state.message.length > 0).toBe(true);
    expect(state.status === 'failed' && state.message).not.toContain('API_SECRET');
    // Save is blocked (not a silent block — the badge shows the reason).
    expect(result.current.mcpConfigModalProps.saveEnabled).toBe(false);
  });

  // Literal-header path is unaffected; validate succeeds
  // and save is enabled when description requirements are also met.
  it('RG-04-savegate (negative): literal headers still validate successfully', async () => {
    const client = createMockClient({
      validateExternalMcpConnection: vi.fn().mockResolvedValue({
        ok: true,
        response: { action: 'externalMcp.validateConnection', mode: 'validated', success: true, message: 'OK' },
    ...
    act(() => { result.current.mcpConfigModalProps.onAdd(); });
    act(() => {
      result.current.mcpConfigModalProps.onDraftChange('display_name', 'Literal Header MCP');
      result.current.mcpConfigModalProps.onDraftChange('purpose', VALID_PURPOSE);
      result.current.mcpConfigModalProps.onDraftChange('preferred_for', VALID_PREFERRED_FOR);
      result.current.mcpConfigModalProps.onDraftChange('url', 'https://mcp.example.com/sse');
      result.current.mcpConfigModalProps.onDraftChange('headers', [{ key: 'X-Custom', value: 'literal-value' }]);
    });
