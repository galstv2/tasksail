...
describe('useAgentConfigModal', () => {
  ...
  it('uses a synthetic provider descriptor to choose the planner-only idle timeout row', async () => {
    const syntheticAgents = [
      {
        agent_id: 'synthetic-planner',
        human_name: 'Avery',
        role_name: 'Planning Strategist',
        required_model: 'gpt-4.1',
        workflow_order: 0,
      },
      {
        agent_id: 'synthetic-builder',
        human_name: 'Blake',
        role_name: 'Implementation Engineer',
        required_model: 'gpt-5.4',
        workflow_order: 1,
      },
    ];
    const describeActiveProvider = vi.fn().mockResolvedValue(createProviderFrontendDescriptor({
      ...
      roster: [
        ...
        { agentId: 'synthetic-builder', roleName: 'Implementation Engineer', humanName: 'Blake', workflowOrder: 2, roleKind: 'builder' },
      ],
    }));
    const loadAgentConfig = vi.fn().mockResolvedValue({
      ok: true,
      response: {
        action: 'agentConfig.loadAgents',
        mode: 'read-only',
        message: 'Loaded synthetic provider assignments.',
    ...
    const client = createMockClient({ describeActiveProvider, loadAgentConfig, saveAgentModels });

    const { result } = renderHook(() => useAgentConfigModal(client), { wrapper });
    act(() => { result.current.openAgentConfigModal(); });
    await waitFor(() => expect(result.current.agentConfigModalProps.agents).toHaveLength(2));

    act(() => {
      result.current.agentConfigModalProps.onAgentIdleTimeoutChange('synthetic-planner', '900');
      result.current.agentConfigModalProps.onAgentIdleTimeoutChange('synthetic-builder', '300');
    });

    await act(async () => { await result.current.agentConfigModalProps.onSave(); });

    const payload = saveAgentModels.mock.calls[0]?.[0] as Array<Record<string, unknown>>;
    expect(payload.find((a) => a.agent_id === 'synthetic-planner')).toMatchObject({ idle_timeout_s: 900 });
    expect(payload.find((a) => a.agent_id === 'synthetic-builder')).not.toHaveProperty('idle_timeout_s');
  });
  ...
  // Skills & Plugins catalog loading.
