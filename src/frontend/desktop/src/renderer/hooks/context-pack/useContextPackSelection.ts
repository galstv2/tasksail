...
import { hydrateLegacyPrimaries, migrateSupportScopes } from '../../components/deep-focus/SidebarDeepFocusUtils';
...
function buildSessionCreatedCatalogEntry(
  ...
  // 'not-run' only comes from stub-tree creation; live seeding overwrites it.
...
export function useContextPackSelection(
  ...
  const handleSelectWorkingFocus = useCallback(
    (focusId: string) => {
      ...
      if (isDistributedEstateMode(selectedPack?.estateType)) {
