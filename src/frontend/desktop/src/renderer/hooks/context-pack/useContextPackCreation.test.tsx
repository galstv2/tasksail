...
describe('useContextPackCreation', () => {
  ...
  it('does not apply defaultContextPackParentDir to existing-project drafts when opening', () => {
    const { result } = renderCreationHook(createClient(), {
    ...
    expect(result.current.contextPackCreationModalProps.draft.creationOrigin).toBe('existing');
    expect(result.current.contextPackCreationModalProps.draft.contextPackDir).toBe('');
  });

  it('uses defaultContextPackParentDir only after switching to new-project creation', () => {
    const { result } = renderCreationHook(createClient(), {
      defaultContextPackParentDir: '/custom/path',
    });
    ...
    act(() => {
      ...
      result.current.contextPackCreationModalProps.onDraftFieldChange('discoveryRoot', '/workspace/orders');
    });

    expect(result.current.contextPackCreationModalProps.draft.contextPackDir).toBe('/custom/path/orders');
  });
  ...
  it('uses the TaskSail default context-pack parent for existing-source discovery', async () => {
    const client = createClient();
    const { result } = renderCreationHook(client, {
      defaultContextPackParentDir: '/Users/operator/TaskSail/contextpacks',
    });
    ...
    await act(async () => {
      await result.current.contextPackCreationModalProps.onDiscoverPrefill();
    });

    expect(result.current.contextPackCreationModalProps.draft.contextPackDir).toBe(
      '/Users/operator/TaskSail/contextpacks/test-pack',
    );
  });

  it('clears generated new-project destination when switching back to existing project', () => {
    const { result } = renderCreationHook(createClient(), {
      defaultContextPackParentDir: '/Users/operator/TaskSail/contextpacks',
    });
    ...
    expect(result.current.contextPackCreationModalProps.draft.contextPackDir).toBe(
      '/Users/operator/TaskSail/contextpacks/orders',
    );

    act(() => {
      result.current.contextPackCreationModalProps.onDraftFieldChange('creationOrigin', 'existing');
    });
  ...
  it('allows distributed shape advancement without an operator primary selection', () => {
    const { result } = renderCreationHook(createClient());
    ...
    // Focus (primary working repo) is no longer an operator choice at creation;
    // the shape step advances without requiring a primary toggle. The default
    // primary flows silently and the manifest derives focus from it.
    expect(result.current.contextPackCreationModalProps.canGoNext).toBe(true);
  ...
  it('serializes repo category fields without standalone repo focus fields', async () => {
    ...
    // Focus is routed via primaryWorkingRepoIds, never a per-entry repositoryType.
    expect(repository).not.toHaveProperty('repositoryType');
    expect(repository).not.toHaveProperty('repoFocus');
    expect(repository).not.toHaveProperty('repoFocusAuthored');
    expect(Array.isArray(createCall?.bootstrapAnswers.primaryWorkingRepoIds)).toBe(true);
  });
  ...
  it('routes monolith focus selection via primaryFocusAreaIds without per-entry repositoryType', async () => {
    const client = createClient();
    const { result } = renderCreationHook(client);
    ...
    const createCall = vi.mocked(client.createContextPack).mock.calls[0]?.[0];
    const focusArea = createCall?.bootstrapAnswers.focusableAreas?.[0];
    expect(focusArea).toEqual(expect.objectContaining({ focusId: 'core' }));
    expect(focusArea).not.toHaveProperty('repositoryType');
    expect(createCall?.bootstrapAnswers.primaryFocusAreaIds).toContain('core');
  });
