...
describe('buildDraftFromDiscovery', () => {
  ...
  it('leaves contextPackDir empty when no TaskSail parent is available', () => {
    const draft: ContextPackCreationDraft = { ...INITIAL_DRAFT, contextPackDir: '' };
    const response = makeDiscoveryResponse();
    const result = buildDraftFromDiscovery(draft, response);
    expect(result.contextPackDir).toBe('');
  });

  it('generates contextPackDir under the TaskSail parent when dir is empty', () => {
    const draft: ContextPackCreationDraft = { ...INITIAL_DRAFT, contextPackDir: '' };
    const response = makeDiscoveryResponse();
    const result = buildDraftFromDiscovery(draft, response, '/Users/operator/TaskSail/contextpacks');
    expect(result.contextPackDir).toBe('/Users/operator/TaskSail/contextpacks/test-pack');
  });
