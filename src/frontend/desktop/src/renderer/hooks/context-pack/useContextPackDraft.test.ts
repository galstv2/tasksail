...
describe('useContextPackDraft hook', () => {
  ...
  it('updateFocusArea sets focusCategoryAuthored when focusCategory changes', () => {
    const { result } = renderHook(() => useTestDraft());
    ...
    act(() => {
      result.current.updateFocusArea(key, 'focusCategory', 'service');
    });

    expect(result.current.draft.focusAreas[0].focusCategory).toBe('service');
    expect(result.current.draft.focusAreas[0].focusCategoryAuthored).toBe(true);
  });
