...
describe('applyScopedRoleAction support-scope mutual exclusion (spec §5.2 / §10)', () => {
  ...
  it('add-global-support strips the path from every per-primary supportTargets bucket', () => {
    ...
    const draft = draftWithPrimaries([
      { ...PRIMARY_TOOLS, supportTargets: [{ path: 'lib/shared', kind: 'directory' }] },
      PRIMARY_PLATFORM,
    ]);
    const next = applyScopedRoleAction(draft, { type: 'add-global-support' }, {
      topLevelId: 'tools',
      target: { path: 'lib/shared', kind: 'directory' },
  ...
  it('add-primary-support strips the path from selectedSupportTargets (globals)', () => {
    const draft = draftWithPrimaries(
  ...
  it('add-primary-support preserves the same path on a sibling primary (cross-primary sharing allowed)', () => {
    // Per spec §10: if two primaries both legitimately need the same support,
    // adding it to one must not silently strip it from the other.
    const draft = draftWithPrimaries([
      { ...PRIMARY_TOOLS, supportTargets: [{ path: 'lib/shared', kind: 'directory' }] },
      PRIMARY_PLATFORM,
    ]);
    const next = applyScopedRoleAction(draft, { type: 'add-primary-support', index: 1 }, {
      topLevelId: 'platform',
      target: { path: 'lib/shared', kind: 'directory' },
      topLevelTargets: [TOOLS, PLATFORM],
      deepFocusMode: 'distributed',
  ...
  it('promote-test-to-global lifts the per-primary test up and clears every matching per-primary entry', () => {
    // promote-test-to-global is the explicit user-initiated coalescing of an
    // already-equivalent test assignment. The reducer must (a) set the global
    // test, (b) strip the matching test from every primary so the canonical
    // form is the global entry alone, and (c) move the cursor to global so the
    // user immediately sees what they just promoted.
    const sharedTest = { path: 'tests/shared', kind: 'directory' as const };
    const draft = draftWithPrimaries([
      { ...PRIMARY_TOOLS, testTarget: sharedTest },
      { ...PRIMARY_PLATFORM, testTarget: sharedTest },
    ]);
    const next = applyScopedRoleAction(draft, { type: 'promote-test-to-global' }, {
      topLevelId: 'tools',
      target: sharedTest,
      topLevelTargets: [TOOLS, PLATFORM],
      deepFocusMode: 'distributed',
  ...
  it('promote-test-to-global clears the same path from selectedSupportTargets to preserve role mutual exclusion', () => {
    // A single path cannot live as both global test and global support. If the
    // user happens to have it in supports, promotion to test must displace it.
    const target = { path: 'tests/shared', kind: 'directory' as const };
    const draft = draftWithPrimaries(
      [
        { ...PRIMARY_TOOLS, testTarget: target },
        { ...PRIMARY_PLATFORM, testTarget: target },
      ],
      [target],
    );
    const next = applyScopedRoleAction(draft, { type: 'promote-test-to-global' }, {
  ...
  it('promote-support-to-global appends to global supports and strips the path from every per-primary bucket', () => {
    const shared = { path: 'lib/shared', kind: 'directory' as const };
    const draft = draftWithPrimaries([
      { ...PRIMARY_TOOLS, supportTargets: [shared] },
      { ...PRIMARY_PLATFORM, supportTargets: [shared] },
    ]);
    const next = applyScopedRoleAction(draft, { type: 'promote-support-to-global' }, {
      topLevelId: 'tools',
      target: shared,
      topLevelTargets: [TOOLS, PLATFORM],
      deepFocusMode: 'distributed',
  ...
  it('promote-support-to-global clears selectedTestTarget if it matches the same path (mutual exclusion)', () => {
    const target = { path: 'lib/shared', kind: 'directory' as const };
    const draft: DeepFocusDraft = {
      ...draftWithPrimaries([
        { ...PRIMARY_TOOLS, supportTargets: [target] },
        { ...PRIMARY_PLATFORM, supportTargets: [target] },
      ]),
      state: {
        ...draftWithPrimaries([
          { ...PRIMARY_TOOLS, supportTargets: [target] },
  ...
  it('moving a support across scopes preserves identity (kind, path)', () => {
    // Start with `assets/icons` as a primary-scoped support; promote to global.
    const draft = draftWithPrimaries([
      { ...PRIMARY_TOOLS, supportTargets: [{ path: 'assets/icons', kind: 'directory' }] },
    ]);
    const promoted = applyScopedRoleAction(draft, { type: 'add-global-support' }, {
      topLevelId: 'tools',
      target: { path: 'assets/icons', kind: 'directory' },
      topLevelTargets: [TOOLS, PLATFORM],
      deepFocusMode: 'distributed',
    }).next;
    ...
    // Now demote the same path back to a primary; globals should clear.
    const demoted = applyScopedRoleAction(promoted, { type: 'add-primary-support', index: 0 }, {
      topLevelId: 'tools',
      target: { path: 'assets/icons', kind: 'directory' },
      topLevelTargets: [TOOLS, PLATFORM],
      deepFocusMode: 'distributed',
