...
import { FALLBACK_PLANNER_DISPLAY, type PlannerDisplayModel } from '../../../shared/agentRoster';
import type { ArchivedTaskChildParentBlockedTip, ArchivedTaskEntry, ContextPackCatalogEntry, ContextPackFocusFilterSelection, MarkdownFileSelection, PlannerFocusValidationIssue, PlannerPersonalityId, PlannerListConversationHistorySummary, StagedDraftContent } from '../../../shared/desktopContract';
import type { ComposerStage, PlannerConversationMessage, PlannerDraftModel } from '../../planner/plannerComposer';
import { classNames } from '../../utils/classNames';
import { BackIcon, CloseIcon } from '../icons';
import SailScreen from './SailScreen';
import MarkdownView from '../shared/MarkdownView';
import ModalShell from '../shared/ModalShell';
import FocusSelectionSummaryCard from '../deep-focus/FocusSelectionSummaryCard';
import { RecentsTrigger } from './RecentsTrigger';
...
import ChildScopeOverridePanel, { type ChildScopeOverridePanelProps } from './ChildScopeOverridePanel';
import type { usePlannerParentArchivePreview } from '../../hooks/planner/usePlannerParentArchivePreview';
import { childScopeToFocusFilterSelection } from '../../planner/plannerChildScope';
import type { PlannerWorkspaceScopeSummary } from '../../planner/plannerWorkspaceScope';
...
export type PlannerModalProps = {
  ...
  plannerDisplay?: PlannerDisplayModel;
  plannerPersonalityId?: PlannerPersonalityId;
  personalityLocked?: boolean;
  onPlannerPersonalityChange?: (id: PlannerPersonalityId) => void;
...
const PERSONALITY_LABELS: Record<PlannerPersonalityId, string> = {
  balanced: 'Balanced',
  clinical: 'Clinical',
};
...
function buildPlannerGreeting(display: PlannerDisplayModel): string {
  return `Hi there! I'm ${display.plannerName}, the ${lowerFirst(display.plannerRoleName)}. Let's figure out what you need.`;
}
...
function PlannerModal({
  ...
  onSendMessage,
  sessionStatus,
  plannerDisplay = FALLBACK_PLANNER_DISPLAY,
  plannerPersonalityId = 'balanced',
  personalityLocked = false,
  onPlannerPersonalityChange,
  ...
  const lockedStyleName = PERSONALITY_LABELS[plannerPersonalityId ?? 'balanced'];
  const personalityLockedHint = buildPersonalityLockedHint(lockedStyleName);
  const plannerName = plannerDisplay.plannerName;
  const plannerGreeting = buildPlannerGreeting(plannerDisplay);
  const composerPlaceholder = childTaskBlocked
    ? CHILD_PARENT_REQUIRED_PLACEHOLDER
    : selectedChildTaskParent
      ? `Tell ${plannerName} what this child task should continue, change, or investigate.`
      : replaySourceRecordId
        ? `Continue this planning thread with ${plannerName}.`
        : `Start a conversation with ${plannerName} to begin planning your task.`;
      ...
      >
        <header className="planner-modal__header">
          <div className="planner-modal__header-left">
            ...
            <h2>{plannerDisplay.plannerRoleName}</h2>
          ...
          <div className="planner-modal__header-right">
            <div
              ...
              data-active={plannerPersonalityId}
            >
              {!personalityLocked && (
                ...
                (['balanced', 'clinical'] as const).map((id) => (
                  <button
                    ...
                    className={classNames(
                      ...
                      plannerPersonalityId === id && 'planner-modal__personality-option--active',
                    )}
                    onClick={() => onPlannerPersonalityChange?.(id)}
                    aria-pressed={plannerPersonalityId === id}
          ...
          {messages.length === 0 && (
            <div className={classNames('planner-msg', 'planner-msg--planner')}>
              <div className="planner-msg__body">
                <MarkdownView content={plannerGreeting} />
              </div>
            </div>
          )}
        ...
        <div className="planner-modal__composer">
          ...
          />
          <button type="button" onClick={onPickMarkdownFile} aria-label="Attach markdown file" className="planner-modal__attach-btn" disabled={!!childTaskBlocked} title={`Attach a markdown file for ${plannerName} to review`}>
        ...
        {sessionStatus === 'active' || sessionStatus === 'busy' || sessionStatus === 'failed' ? (
          <div className="planner-modal__actions">
            ...
            <button
              ...
              title={`Ask ${plannerName} to write a planning spec from the conversation so far`}
            >
            ...
            <button
              ...
              title={`Review the spec ${plannerName} drafted`}
            >
              View Draft
            </button>
            <button
              ...
              title={`Accept ${plannerName}'s draft and submit to the task queue`}
            >
