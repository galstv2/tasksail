...
import { createLocalDraft } from '../../planner/plannerComposer';
import type { PlannerConversationMessage } from '../../planner/plannerComposer';

const LILY_PLANNER_DISPLAY = {
  plannerName: 'Lily',
  plannerDisplayName: 'Lily (Planning Specialist)',
  plannerRoleName: 'Planning Specialist',
};

const AVERY_PLANNER_DISPLAY = {
  plannerName: 'Avery',
...
function makeProps(overrides: Partial<PlannerModalProps> = {}): PlannerModalProps {
  return {
    ...
    messages: [],
    onSendMessage: vi.fn(),
    plannerDisplay: LILY_PLANNER_DISPLAY,
    ...overrides,
  };
}
...
describe('PlannerModal', () => {
  ...
  it('renders active planner copy from the supplied display model', () => {
    render(<PlannerModal {...makeProps({ plannerDisplay: AVERY_PLANNER_DISPLAY })} />);

    expect(screen.getByRole('heading', { name: 'Planning Strategist' })).toBeInTheDocument();
    expect(screen.getByText(/I'm Avery, the planning strategist/i)).toBeInTheDocument();
    expect(screen.getByLabelText('Planner message input')).toHaveAttribute(
      'placeholder',
      'Start a conversation with Avery to begin planning your task.',
    );
    expect(screen.queryByText(/I'm Lily/i)).not.toBeInTheDocument();
  });
  ...
  it('renders and changes the Lily personality control before the first message', () => {
    const onPlannerPersonalityChange = vi.fn();
    render(<PlannerModal {...makeProps({ plannerPersonalityId: 'balanced', onPlannerPersonalityChange })} />);
    ...
    expect(onPlannerPersonalityChange).toHaveBeenCalledWith('clinical');
  });
  ...
  it('removes personality buttons after the first message and names the locked style', () => {
    ...
    expect(onPlannerPersonalityChange).not.toHaveBeenCalled();
  });
