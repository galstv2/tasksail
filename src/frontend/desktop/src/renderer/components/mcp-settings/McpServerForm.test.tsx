...
describe('McpServerForm', () => {
  ...
  // Env-ref validation failure messages must be operator-visible
  // through the ValidationBadge, not silently hidden.
  it('RG-04-savegate: env-ref failed validation message is shown in the badge', () => {
    const envRefMessage = 'Headers use environment-variable references and cannot be validated before the server is saved. Save the server first, then re-open it to validate the live connection.';
    render(<McpServerForm {...defaultProps({
      connectionValidation: { status: 'failed', message: envRefMessage },
      saveEnabled: false,
    })} />);
    // The failed badge renders the message — operator-visible, not silent.
    expect(screen.getByText(envRefMessage)).toBeTruthy();
    // Save remains disabled.
    expect((screen.getByText('Save') as HTMLButtonElement).disabled).toBe(true);
  });

  // Literal-header success state shows a Connected badge and enables Save.
  it('RG-04-savegate (negative): literal-header success state shows Connected and enables Save', () => {
    render(<McpServerForm {...defaultProps({
      connectionValidation: { status: 'success', message: 'OK' },
      saveEnabled: true,
    })} />);
