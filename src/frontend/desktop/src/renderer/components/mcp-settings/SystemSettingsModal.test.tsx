...
describe('SystemSettingsModal', () => {
  ...
  it('renders debug and malformed records as literal text with pager controls', () => {
    ...
    expect(screen.getByText(/rows 4-9/)).toBeInTheDocument();
    expect(screen.getByText('Rows 4-9')).toBeInTheDocument();
    const localTimestamp = expectedLocalTimestamp('2026-06-03T10:00:00.000Z');
    expect(screen.getByText(localTimestamp)).toBeInTheDocument();
    expect(screen.getByText((_content, element) =>
      element?.classList.contains('system-settings__log-meta') === true
        && element.textContent?.includes(localTimestamp) === true,
    )).toBeInTheDocument();
  ...
  it('sorts log rows by timestamp descending and expands a clicked row', () => {
    ...
    expect(screen.getAllByText(/Row [123]/).map((node) => node.textContent)).toEqual([
      'Row 2',
      'Row 3',
      'Row 1',
    ]);
    expect(screen.getByText(expectedLocalTimestamp('2026-06-03T10:02:00.000Z'))).toBeInTheDocument();
