...
function buildCommandStripActions(
  ...
  // scope the row already belongs to (spec §6.1).
  ...
  // `add-global-support` in that case, but the spec (§6.1) wants the cluster
  // visible with an explanatory tooltip so the scope choice remains discoverable.
