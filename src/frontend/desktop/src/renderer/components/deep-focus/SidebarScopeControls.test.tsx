...
describe('SidebarScopeControls', () => {
  ...
  it('renders repositories subtitle for distributed packs and focus areas for monolith packs', () => {
    const { rerender } = render(
      <SidebarScopeControls
        {...defaultProps}
        selectedPack={makePack({ estateType: 'distributed' })}
      />,
    );
  ...
  it('restores the repo Deep Focus primary when re-enabling a plain distributed pack', () => {
    const onCommitDeepFocusSelection = vi.fn();
  ...
  it('renders compact Deep Focus summary and opens the editor', async () => {
    ...
    render(
      <ScopeControlsWithEditor
        ...
        // "Edit Scope" empty-state CTA. The previous fixture
  ...
  it('commits working Deep Focus selection from the editor', async () => {
    ...
    expect(onCommitDeepFocusSelection).toHaveBeenCalledWith(
      expect.objectContaining({
        deepFocusEnabled: true,
        // Scalar holds the source primary manifest `repoId`
  ...
  it('does not ship row popover artifacts', () => {
    ...
    const componentDir = 'src/renderer/components/deep-focus';
  ...
  it('keeps folder disclosure clicks larger than the chevron glyph', () => {
    const css = readFileSync('src/renderer/styles/sidebar/deep-focus/deep-focus-tree-row.css', 'utf-8');

    expect(css).toContain('grid-template-columns: 14px minmax(0, 1fr) minmax(var(--ts-btn-sm), 1.75rem);');
    expect(css).toMatch(/\.deep-focus-row__chevron\s*\{[^}]*width: 100%;[^}]*min-height: var\(--ts-btn-sm\);/s);
    expect(css).toContain('.deep-focus-row__chevron--expanded svg');
  });
