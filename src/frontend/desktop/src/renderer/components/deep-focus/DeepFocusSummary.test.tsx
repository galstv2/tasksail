...
import type { EditScopeCursor } from './SidebarDeepFocusUtils';
...
function renderSummary({
  ...
  onOpenEditor = vi.fn<(cursor?: EditScopeCursor) => void>(),
  actionRef,
}: {
  ...
  onOpenEditor?: Mock<(cursor?: EditScopeCursor) => void>;
  actionRef?: ReturnType<typeof createRef<HTMLButtonElement>>;
}): { onOpenEditor: Mock<(cursor?: EditScopeCursor) => void>; container: HTMLElement } {
  const result = render(
    <DeepFocusSummary
...
describe('DeepFocusSummary', () => {
  ...
  it('renders a globals block with only a test row when only globalTest is set', () => {
    ...
    expect(globals?.getAttribute('aria-label')).toBe('Test for all primaries');
    expect(
      globals?.querySelector('.deep-focus-summary__globals-eyebrow')?.textContent,
    ).toBe('Test for all primaries');
  ...
  it('labels the globals section as "Support for all primaries" when global supports exist', () => {
    ...
    expect(globals?.querySelector('.deep-focus-summary__globals-label')?.textContent).toBe('Support');
  });

  it('renders global test and support as separate labeled rows', () => {
    const { container } = renderSummary({
      committedPrimaries: [
        {
          path: 'services/Acme.Api',
          kind: 'directory',
          role: 'anchor',
          repoLocalPath: '/repos/platform',
          repoId: 'platform',
    ...
    const globals = container.querySelector('.deep-focus-summary__globals');
    const rows = globals?.querySelectorAll('.deep-focus-summary__globals-row');
    expect(globals?.getAttribute('aria-label')).toBe('Scope for all primaries');
    expect(
      globals?.querySelector('.deep-focus-summary__globals-eyebrow')?.textContent,
    ).toBe('Scope for all primaries');
    expect(globals?.textContent).not.toContain('Support for all primaries');
    expect(rows).toHaveLength(2);
    expect(rows?.[0]?.querySelector('.deep-focus-summary__globals-label')?.textContent).toBe('Test');
    expect(rows?.[0]?.textContent).toContain('Acme.Api.Tests');
    expect(rows?.[0]?.textContent).not.toContain('Acme.Models');
    expect(rows?.[1]?.querySelector('.deep-focus-summary__globals-label')?.textContent).toBe('Support');
    expect(rows?.[1]?.textContent).toContain('Acme.Models');
    expect(rows?.[1]?.textContent).not.toContain('Acme.Api.Tests');
  });
