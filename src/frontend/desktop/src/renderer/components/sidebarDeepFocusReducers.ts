 ...
 *
 * Per spec §2.3, `selectedWorkingFocusIds` holds manifest IDs (e.g. `tools`),
 * not `repoLocalPath` strings. Callsites that look up a `TopLevelTarget` from
    ...
    const nextFocusTargets = existingIndex >= 0
      ...
      : normalizePrimaryTargetRoles([
        ...prunedBaseTargets,
        {
          ...
          // Per spec §2.1: stamp both `repoLocalPath` (for filesystem-bound
...
export function derivePrimaryIds(
  ...
  // Per spec §2.4: scalars hold manifest IDs (repoId/focusId), NOT repoLocalPath.
