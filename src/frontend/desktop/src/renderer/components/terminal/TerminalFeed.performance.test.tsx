...
/** 500 events where task 0 gets 490 (noisy) and each other task gets 2. */
...
describe('TerminalFeed 10-task scope and replay coverage', () => {
  ...
  it('scoped replay after noisy task: select quiet task scope, events are scoped correctly', () => {
    ...
    // Quiet task has 2 events: makeEvent(0, 1) → id "evt-1-0", makeEvent(1, 1) → id "evt-1-1"
    ...
    const triggerAfter = screen.getByRole('button', { name: 'Terminal task scope' });
    expect(triggerAfter.textContent).toContain(quietScope.title ?? '');
