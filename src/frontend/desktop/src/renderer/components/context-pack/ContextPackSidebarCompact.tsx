...
function ContextPackSidebarCompact({
  ...
  const activePack = contextPacks.find(
    (pack) => pack.contextPackDir === activeContextPackDir || pack.isActive,
  );
  const fallbackReadyPack = contextPacks.find((pack) => pack.bootstrapReady);
  const visibleContextPacks = activePack ? [activePack] : fallbackReadyPack ? [fallbackReadyPack] : [];
      ...
      <nav className="sidebar-icons" aria-label="Context pack actions">
        ...
        {visibleContextPacks.map((pack) => (
          <button
            ...
            className={classNames(
              'sidebar-icon-btn',
              (pack.contextPackDir === activeContextPackDir || pack.isActive) && 'sidebar-icon--active',
            )}
