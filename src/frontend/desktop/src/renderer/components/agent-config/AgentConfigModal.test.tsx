...
import type { AgentConfigModalProps } from '../../hooks/agent-config/useAgentConfigModal';
...
describe('AgentConfigModal', () => {
  ...
  it('renders synthetic provider planner labels from the descriptor roster', () => {
    const descriptor = createProviderFrontendDescriptor({
    ...
    render(<AgentConfigModal {...defaultProps({
      agents: [
        {
          ...
          workflow_order: 0,
          options: [{ display_name: 'GPT 5.4', model_id: 'gpt-5.4' }],
          effortOptions: ['none', 'medium', 'high'],
          effortDisabled: false,
          currentModelMissing: false,
          selectedExtensionIds: [],
          selectedExternalMcpIds: [],
      ...
      models: [{ display_name: 'GPT 5.4', model_id: 'gpt-5.4', usageCount: 1, inUseBy: ['Avery'] }],
      descriptor,
    })} />);

    expect(screen.getByText('Avery')).toBeTruthy();
    expect(screen.getByText('Planning Strategist')).toBeTruthy();
    expect(screen.getByLabelText('Avery idle timeout (s)')).toBeInTheDocument();
    expect(screen.getByLabelText('Avery reasoning effort')).toBeEnabled();
  });
