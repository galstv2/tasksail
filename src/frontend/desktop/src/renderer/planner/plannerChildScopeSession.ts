import type {
  ArchivedTaskEntry, PlannerChildTaskExecutionScope, PlannerFocusSnapshot, PlannerPersonalityId,
  PlannerPlanningReloadScope, PlannerParentBranchViewRequest, PlannerReadParentChainArchiveBundleResponse,
...
const log = createLogger('src/renderer/plannerChildScopeSession');
...
export function buildParentTaskBranchViewRequest(task: ArchivedTaskEntry): PlannerParentBranchViewRequest | undefined {
  ...
  try {
    ...
    log.warn('plannerParentBranchView.coverage.assessment-failed', { parentTaskId: task.taskId });
  }
  return {
    schemaVersion: 1,
...
function emitParentBranchHandoffCoverageWarning(gap: ParentBranchHandoffCoverageGap): void {
  log.warn('plannerParentBranchView.coverage.partial', gap);
}
...
export async function restartChildPlannerWithScope(args: {
  ...
  const { client, task, childScope, reloadScope, plannerPersonalityId, parentContextBundle, onBeforeStart, onStatus, onStarted } = args;
  if (!task.plannerFocusSnapshot) {
  ...
  const start = await client.startPlannerSession({
    ...
    plannerPlanningReloadScope: reloadScope,
