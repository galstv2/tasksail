...
export function selectedWorkingFocusIdsForScope(
  ...
  return isDistributedEstateMode(selectedPack.estateType)
...
export function updateStandardChildScope(
  ...
  const key = isDistributedEstateMode(selectedPack.estateType) ? 'selectedRepoIds' : 'selectedFocusIds';
...
export function updateChildScopeDeepFocus(
  ...
  return {
    ...
    deepFocusPrimaryRepoId: isDistributedEstateMode(selectedPack.estateType)
      ? selection.deepFocusPrimaryRepoId
      : null,
    deepFocusPrimaryFocusId: isDistributedEstateMode(selectedPack.estateType)
      ? null
...
export function buildPlannerPlanningReloadScope(
  ...
  const output: PlannerPlanningReloadScope = {
  ...
  assertPlannerPlanningReloadScopePostcondition(parentScope, childScope, output);
  return output;
}
...
function assertPlannerPlanningReloadScopePostcondition(
  parentScope: PlannerChildTaskExecutionScope,
  childScope: PlannerChildTaskExecutionScope,
  output: PlannerPlanningReloadScope,
): void {
  const fail = (field: string, id?: string): never => {
    throw new Error(`Planner planning reload scope postcondition failed for ${childScope.contextPackId}: ${field}${id ? `:${id}` : ''}.`);
  };
  ...
  if (output.purpose !== 'planner-planning-read-context') fail('purpose');
...
export const __plannerChildScopeTestHooks = {
  assertPlannerPlanningReloadScopePostcondition,
};
