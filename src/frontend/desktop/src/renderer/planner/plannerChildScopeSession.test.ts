...
describe('plannerChildScopeSession parent branch handoff coverage', () => {
  ...
  it('warns for distributed-platform primary repos missing branch handoff coverage', () => {
    ...
    expect(loggerMock.warn).toHaveBeenCalledWith('plannerParentBranchView.coverage.partial', {
      parentTaskId: 'TASK-1',
      contextPackId: 'orders',
    ...
    expect(Object.keys(loggerMock.warn.mock.calls[0]![1] as Record<string, unknown>)).toEqual([
      'parentTaskId',
      'contextPackId',
      'estateType',
    ...
    expect(JSON.stringify(loggerMock.warn.mock.calls[0]![1])).not.toContain('Sensitive');
    expect(JSON.stringify(loggerMock.warn.mock.calls[0]![1])).not.toContain('/archives');
    expect(JSON.stringify(loggerMock.warn.mock.calls[0]![1])).not.toContain('task/parent');
    expect(JSON.stringify(loggerMock.warn.mock.calls[0]![1])).not.toContain('head');
  });
  ...
  it('keeps request construction alive when coverage assessment fails', () => {
    ...
    expect(loggerMock.warn).toHaveBeenCalledWith('plannerParentBranchView.coverage.assessment-failed', {
      parentTaskId: 'TASK-1',
    });
  });
});
