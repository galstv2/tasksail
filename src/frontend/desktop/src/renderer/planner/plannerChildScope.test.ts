...
describe('plannerChildScope', () => {
  ...
  it('builds deterministic read context while preserving child execution authority', () => {
    ...
    const reload = buildPlannerPlanningReloadScope(parentScope, childScope, selectedPack);

    expect(reload.schemaVersion).toBe(1);
    expect(reload.purpose).toBe('planner-planning-read-context');
  ...
  it('uses repo IDs for plain distributed child-scope updates', () => {
    const plainDistributedPack = { ...selectedPack, estateType: 'distributed' };
    const scope = {
      ...parentScope,
      deepFocusEnabled: false,
      selectedRepoIds: ['orders-api'],
      selectedFocusIds: ['legacy-focus'],
      selectedFocusTargets: [],
      selectedTestTarget: null,
      selectedSupportTargets: [],
    };

    const standard = updateStandardChildScope(plainDistributedPack, scope, 'billing-api');
    expect(standard.selectedRepoIds).toEqual(['orders-api', 'billing-api']);
    expect(standard.selectedFocusIds).toEqual(['legacy-focus']);

    const deepFocus = updateChildScopeDeepFocus(plainDistributedPack, scope, {
      deepFocusEnabled: true,
      deepFocusPrimaryRepoId: 'orders-api',
      deepFocusPrimaryFocusId: 'legacy-focus',
      selectedFocusPath: null,
      selectedFocusTargetKind: null,
      selectedFocusTargets: [],
      selectedTestTarget: null,
    ...
    expect(deepFocus.deepFocusPrimaryRepoId).toBe('orders-api');
    expect(deepFocus.deepFocusPrimaryFocusId).toBeNull();
  });
  ...
  it('fails closed for cross-context-pack reload scope unions', () => {
    expect(() => buildPlannerPlanningReloadScope(parentScope, {
      ...parentScope,
      contextPackId: 'other',
  ...
  it('hydrates monolith Deep Focus read-context targets by focus ID', () => {
    ...
    const reload = buildPlannerPlanningReloadScope(parentMonolithScope, childMonolithScope, monolithPack);
