...
<!-- Before drafting, ingest the Guide's ask, platform-provided planning context, operator-selected focus or execution scope, parent context when present, context-pack focus metadata, and linked evidence. Scale intake detail to task size and risk: keep simple surgical tasks concise and exact; add file paths, symbols, compatibility notes, validation, and routing rationale for medium, complex, risky, or cross-cutting tasks only when those details reduce ambiguity or prevent regressions. Do not add filler. -->

## Request Summary
<!-- (2+ sentences; minimum 20 characters) — say what the task is, why it is being asked for, and what concrete result is wanted. Use the Guide's exact terms when possible. Include any behavior, artifact, or workflow that must stay unchanged. Do not turn this section into codebase research; put file paths, symbols, and technical anchors in Constraints, Critical Requirements, Compatibility Requirements, Required Validation, or Acceptance Signals when they reduce ambiguity. -->
...
<!-- (required for "child-task"; leaving this blank when Task Kind is child-task fails validation. Leave blank for "standard".) Record only the parent context that still matters: preserved decisions, inherited constraints, unresolved risks, and what changed from the parent task. Child Execution Scope is implementation authority; parent context and additional planning context are read-only background unless the platform explicitly stages them as execution scope. -->
